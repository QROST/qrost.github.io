...
setTimeout(() => {
  ...
  T('listings 147', (w.HOUSING_LISTINGS || []).length === 147);
  T('enriched 147', Object.keys(w.HOUSING_ENRICHED || {}).length === 147);
  T('hazards 21', Object.keys(w.HOUSING_HAZARDS || {}).length === 21);
  ...
  T('table count 122 default', /显示 122 \/ 122/.test(ids['table-count'].textContent));
  ids['tier1-toggle'] = { checked: false, addEventListener() {} };
  T('tier1 toggle wired', typeof w.__setTier1On === 'function');
  try {
    w.__setTier1On(true);
    T('table count 147 tier1', /显示 147 \/ 147/.test(ids['table-count'].textContent));
