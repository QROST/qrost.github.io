...
setTimeout(() => {
  ...
  T('enriched 392', Object.keys(w.HOUSING_ENRICHED || {}).length === 392);
  T('hazards 33', Object.keys(w.HOUSING_HAZARDS || {}).length === 33);
  ...
  T('table count 249 default', /显示 249 \/ 249/.test(ids['table-count'].textContent));
  ids['tier1-toggle'] = { checked: false, addEventListener() {} };
  T('tier1 toggle wired', typeof w.__setTier1On === 'function');
  try {
    w.__setTier1On(true);
    T('table count 392 tier1', /显示 392 \/ 392/.test(ids['table-count'].textContent));
