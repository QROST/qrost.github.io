...
(function () {
  ...
  const LABELS = {
    zh: {
      ...
      rankNoteExtreme: '<strong class="text-slate-700 dark:text-slate-300">极端日</strong>：同一日气候口径下，<strong class="text-slate-700 dark:text-slate-300">日均 &lt;-5℃（严寒）</strong>或<strong class="text-slate-700 dark:text-slate-300">日最高 ≥33℃（酷热，用未平滑日最高判定）</strong>；若历史最高温比曲线峰值高 ≥3℃，按缺口补计若干酷热日（如 HK/TW 格点平滑后偏低）。下方红条为全年极端时段。<strong class="text-slate-700 dark:text-slate-300">宜居口径</strong>，非气象预警。',
      ...
      methodComfortExtremeBody: '<li><strong class="text-slate-700 dark:text-slate-300">数据</strong>：Open-Meteo ERA5 <strong>2014–2023</strong> 逐日气温 → 年内同日期做 <strong>10 年平均</strong> → 曲线展示用 <strong>15 天圆周平滑</strong>（固定 365 天历法）。</li><li><strong class="text-slate-700 dark:text-slate-300">舒适日</strong>：平滑后 <strong>日最低温 ≥8℃ 且日最高温 ≤26℃</strong>（全天温区落在舒适带内），且<strong>当天不算极端</strong>。</li><li><strong class="text-slate-700 dark:text-slate-300">极端日</strong>：平滑后 <strong>日均温 &lt; -5℃</strong>（严寒；0～-4℃ 干冷不计）或 <strong>未平滑日最高温 ≥ 33℃</strong>（酷热；比气象 35℃ 预警线更贴近日常体感不适）。</li><li><strong class="text-slate-700 dark:text-slate-300">历史高温补计</strong>：若表格「历史最高温」比平滑曲线峰值高 <strong>≥3℃</strong>（沿海 HK/TW 常见），按温差补计酷热日并从舒适日中扣除，保持互斥。</li><li><strong class="text-slate-700 dark:text-slate-300">互斥</strong>：舒适与极端<strong>不重叠</strong>——已标为极端的日子不再计舒适（表格绿/红条、排行天数均如此）。</li><li><strong class="text-slate-700 dark:text-slate-300">性质</strong>：<strong>宜居对比口径</strong>，非中国气象局灾害预警；日期边界因平滑约有 <strong>±1 周</strong> 不确定度。</li>',
...
methodDataTitle: '数据来源与整合',
    ...
    en: {
      ...
      rankNoteExtreme: '<strong class="text-slate-700 dark:text-slate-300">Extreme days</strong>: same climatology — <strong class="text-slate-700 dark:text-slate-300">mean &lt; -5°C (severe cold)</strong> or <strong class="text-slate-700 dark:text-slate-300">unsmoothed daily high ≥33°C (heat)</strong>; when record high exceeds the curve peak by ≥3°C (common on coastal HK/TW grids), extra heat-stress days are added. Red bar = extreme spans. <strong class="text-slate-700 dark:text-slate-300">Livability thresholds</strong>, not CMA warnings.',
