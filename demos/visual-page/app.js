...
import { Sonifier } from './audio.js?v=caeb2876df';   // 生成式数据音乐引擎（zero-dep Web Audio）
...
addEventListener('resize', () => { camera.aspect = innerWidth / innerHeight; camera.updateProjectionMatrix(); renderer.setSize(innerWidth, innerHeight); U.uPixelRatio.value = renderer.getPixelRatio(); beamMaterial.uniforms.uRes.value.set(innerWidth, innerHeight); edgeRibbonMat.uniforms.uRes.value.set(innerWidth, innerHeight); });
