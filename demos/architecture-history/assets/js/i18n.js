...
(function () {
  ...
  const STRINGS = {
    ...
    heroLead: {
      ...
      en: 'A traceable historical graph connecting works, architects, practices, and knowledge-transfer records. The current release is a cross-regional candidate sample for validating the data pipeline—not a complete global history.',
    },
    ...
    metricWorksSub: { zh: '跨 9 个宏观区域的种子作品', en: 'seed works across 9 macroregions' },
    ...
    boundaryCopy: {
      zh: '当前目录是事先选定、并钉死 Wikidata 修订版本的种子作品：用来校验映射与溯源流程，不是全球全库普查。右侧 9×8 发现矩阵做加深抽样侦察（每格最多 8 条，按从新到老分波推进），不会自动整库导入；人物、事务所与关系的审核富集在这份种子目录上继续加密。',
      en: 'The catalog is curated seed works pinned to exact Wikidata revisions—used to validate mapping and provenance, not a world census. The 9×8 discovery matrix deepens scouting (≤8 works per cell, newest-to-oldest waves) and does not bulk-import the world; person, practice, and relation review densifies on this seed catalog.',
    },
    ...
    mapTouchEnable: { zh: '移动地图', en: 'Move map' },
    mapTouchDisable: { zh: '锁定地图（可滚动页面）', en: 'Lock map (scroll page)' },
    graphTouchEnable: { zh: '移动关系图', en: 'Move graph' },
    graphTouchDisable: { zh: '锁定关系图（可滚动页面）', en: 'Lock graph (scroll page)' },
    ...
    mapNote: { zh: '当前 {withCoordinates}/{total} 件作品带来源坐标；其余 {missing} 件保持缺失，不猜测补齐。', en: '{withCoordinates} of {total} works carry source coordinates; the remaining {missing} stay explicitly missing with no guessed coordinates.' },
    ...
    catalogPaginationAria: { zh: '目录分页', en: 'Catalog pagination' },
    catalogPrevious: { zh: '上一页', en: 'Previous' },
    catalogNext: { zh: '下一页', en: 'Next' },
    catalogPageStatus: { zh: '当前显示 {start}–{end}，筛选结果共 {filtered} 条', en: 'Showing {start}–{end} of {filtered} filtered records' },
    lineageKicker: { zh: '03 · 待审知识传递', en: '03 · KNOWLEDGE-TRANSFER REVIEW' },
    lineageTitle: { zh: '原始关系复核图', en: 'Raw relation review graph' },
    lineageIntro: {
      zh: '图中虚线只展示人物间的学习与影响线索；右侧清单还可筛选事务所任职、门下任职与共同创立。全部关系均为来源记录的候选线索，不能自动证明师承。',
      en: 'The dashed graph shows only person-to-person study and influence clues. The semantic list can also filter practice employment, employment-under, and co-founding. Every relation remains a source-recorded candidate and cannot by itself establish lineage.',
    },
    ...
    lineageTypePractice: { zh: '事务所任职', en: 'Practice employment' },
    lineageTypeWorkedFor: { zh: '门下任职', en: 'Employment under' },
    ...
    lineageWarningCopy: {
      ...
      en: 'This graph shows person-to-person knowledge-transfer clues only; practice employment and founding appear in person details. No source record automatically establishes teaching, mentorship, or apprenticeship; every edge requires human review.',
    },
    ...
    coverageIntro: {
      zh: '发现矩阵共 72 个区域×时期单元，回答“去 Wikidata 侦察时能不能找到候选”；每格最多抽 8 条作为发现回执，并按从新到老分波扩入种子目录。它不等于全球全库，也不替代对人物、建筑、事务所与关系的逐条审核富集。',
      en: 'The discovery matrix has 72 region × period cells and answers whether Wikidata yields candidates (≤8 scouted receipts each), promoted into the seed catalog in newest-to-oldest waves. It is not a world census and does not replace person, work, practice, and relation review.',
    },
    ...
    emptyObserved: { zh: '已查无结果', en: 'Empty observed' },
    sampled: { zh: '已抽样', en: 'Sampled' },
    blocked: { zh: '受阻', en: 'Blocked' },
    partial: { zh: '部分运行', en: 'Partial' },
    complete: { zh: '已运行', en: 'Run' },
    fixtureTitle: { zh: '种子作品的区域分布', en: 'Regional distribution of seed works' },
    fixtureCopy: {
      zh: '柱状图只说明这批钉死修订的种子作品落在哪些宏观区域，用来验管线；不等于该区域已普查完成。',
      en: 'The bars show where these revision-pinned seed works land by macroregion—for pipeline validation, not a claim that each region has been fully surveyed.',
    },
    ...
    methodOneCopy: { zh: '优先接入 Wikidata、Getty Vocabularies、Archnet、GAHTC 等已登记的外部知识组织。证据只接受 independent_external 发布者；QROST 自写研究简报不作为信息来源或证据。', en: 'Prioritize registered external knowledge organizations such as Wikidata, Getty Vocabularies, Archnet, and GAHTC. Evidence accepts only independent_external publishers; research briefs authored by QROST are never an information source or evidence authority.' },
    ...
    sourceRegistryCopy: { zh: '显示计划复用的来源、许可决策与当前适配状态。当前公开人物记录含 {count} 个 ULAN 外部标识；每个标识仍是身份候选，不构成关系证据。', en: 'Shows planned reusable sources, rights decisions, and adapter state. Current public people records contain {count} ULAN external identifiers; each remains an identity candidate and is not relationship evidence.' },
    licenseTitle: { zh: '许可与快照边界', en: 'Rights and snapshot boundary' },
    licenseCopy: { zh: '结构化 Wikidata 数据按 CC0 复用，Getty ULAN 数据按 ODC-By 1.0 复用；页面保留固定修订或响应哈希。其他来源只有在登记的 allowed_operations 允许时才会进入发布数据。', en: 'Structured Wikidata data is reused under CC0 and Getty ULAN data under ODC-By 1.0; pinned revision or response hashes are retained. Other sources enter published data only when their registered allowed_operations permit it.' },
    ...
    detailRoles: { zh: '来源职业映射', en: 'Source occupation mapping' },
    detailCredits: { zh: '候选署名', en: 'Candidate credits' },
    detailWorks: { zh: '关联作品', en: 'Linked works' },
    detailPracticeAffiliations: { zh: '事务所 / Practices', en: 'Practices' },
    detailRelations: { zh: '待审知识传递线索', en: 'Knowledge-transfer review clues' },
    detailClaims: { zh: '字段级证据主张', en: 'Field-level evidence claims' },
    claimsLoading: { zh: '正在按需核验并载入 {count} 条字段级主张…', en: 'Verifying and loading {count} field-level claims on demand…' },
    claimsUnavailable: { zh: '字段级证据暂时无法载入；实体与关系摘要仍可浏览，未绕过完整性校验。', en: 'Field-level evidence is temporarily unavailable. Entity and relation summaries remain usable, and integrity checks were not bypassed.' },
    claimsRetry: { zh: '重试载入证据', en: 'Retry evidence load' },
    ...
    fixtureOnly: { zh: '仅离线样例', en: 'Offline sample only' },
  ...
  const ENUMS = {
    ...
    role: {
      ...
      master_builder: { zh: '营造师', en: 'Master builder' },
      engineer: { zh: '工程师', en: 'Engineer' },
      landscape_architect: { zh: '景观建筑师', en: 'Landscape architect' },
      urban_planner: { zh: '城市规划师', en: 'Urban planner' },
      designer: { zh: '设计师', en: 'Designer' },
      educator: { zh: '教育者', en: 'Educator' },
      craftsperson: { zh: '工艺从业者', en: 'Craftsperson' },
      patron: { zh: '赞助人', en: 'Patron' },
      historian: { zh: '历史学者', en: 'Historian' },
      unknown: { zh: '未映射职业', en: 'Unmapped occupation' },
    },
    rejection_reason: {
    ...
    adapter_status: {
      fixture_only: { zh: '仅离线样例', en: 'Offline sample only' },
