...
(function () {
  ...
  const STRINGS = {
    ...
    truthCopy: {
      zh: '现有记录全部来自固定版本的 Wikidata 水合样本，尚未经过本项目人工编辑核验。师承边仅供复核，不能进入游戏升级树。',
      en: 'Every current record comes from revision-pinned Wikidata hydration fixtures and has not received this project’s human editorial verification. Relation clues are review-only and cannot enter a game upgrade tree.',
    },
    ...
    boundaryCopy: {
      zh: '19 件作品是校验来源映射的固定候选夹具，不是排名、代表性抽样或全球覆盖率。所有 72 个“区域 × 时期”发现单元仍未运行。',
      en: 'The 19 works are fixed candidate fixtures for validating source mappings—not a ranking, representative sample, or global coverage measure. All 72 region × period discovery cells remain unrun.',
    },
    ...
    mapNote: { zh: '18/19 件夹具作品带来源坐标；波斯波利斯保持坐标缺失。', en: '18 of 19 fixture works carry source coordinates; Persepolis remains explicitly missing.' },
    ...
    catalogIntro: {
      zh: '中英文名、别名、Wikidata ID、地点与署名均可搜索。搜索、区域与证据筛选同步驱动地图和目录；实体类型标签只整理目录。',
      en: 'Search Chinese and English names, aliases, Wikidata IDs, places, and credits. Search, region, and evidence filters drive both map and catalog; entity-type tabs organize the catalog only.',
    },
    ...
    lineageWarningTitle: { zh: '不是游戏升级树。', en: 'This is not a game upgrade tree.' },
    lineageWarningCopy: {
      zh: '只有经人工核验、无冲突且关系类型明确的边，未来才可能由游戏项目另行派生。',
      en: 'Only human-verified, conflict-free, explicitly typed relations may later be derived by the separate game project.',
    },
    ...
    methodFourTitle: { zh: '游戏只读已核验子图', en: 'The game reads only a verified subgraph' },
    methodFourCopy: { zh: '第二个 demo 将按独立、版本化规则读取无冲突的已核验关系，而不修改历史数据。', en: 'The second demo will read conflict-free verified relations through separate, versioned rules without mutating historical data.' },
