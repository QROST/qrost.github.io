...
def load_crosswalk_snapshot():
    """Return the committed ULAN crosswalk snapshot, or None if absent.

    The crosswalk snapshot is gitignored local infrastructure regenerated by
    fetch_wikidata_ulan_crosswalk.py. After significant catalog edits (entity
    additions/deletions, region backfills) it becomes stale and is removed
    until the next networked re-fetch. Tests that depend on it should call
    this helper and skip themselves when it returns None.
    """
...
class WikidataUlanCrosswalkTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.snapshot = load_crosswalk_snapshot()

    def setUp(self):
        if self.snapshot is None:
            self.skipTest("no committed ULAN crosswalk snapshot (run fetch_wikidata_ulan_crosswalk.py)")
...
class GettyUlanPilotTests(unittest.TestCase):
    ...
    def test_all_exact_screening_accepts_all_24_and_hashes_are_recomputable(self):
        crosswalk_snapshot = load_crosswalk_snapshot()
        if crosswalk_snapshot is None:
            self.skipTest("no committed ULAN crosswalk snapshot")
