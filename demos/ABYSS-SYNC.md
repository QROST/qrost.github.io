...
## 5b. 悦耳度 pass（2026-07-16，lofi 单侧；owner 反馈"略微刺耳"）

流程：5 视角专业评审（母带/心理声学/lofi 制作人/爵士配器/WebAudio DSP，全 opus）
→ 综合实施 → 2 视角对抗验证（回归猎手 APPROVE + 过度矫正裁判 APPROVE_WITH_TWEAKS）
→ round-2 微调。全程用 `tools/measure-harshness.html`（新增，确定性离线渲染测量）
出 before/after 数字：尖锐度 −17.1..−19.7dB → −20.9..−24.0dB，clicks/min 13–71 → ~0，
RMS ±0.2dB（响度不变）。`_rng` 棘轮 18 不变；harmony/diversity 门全绿。

| # | 改动 | lofi commit | neon 侧评估 |
|---|------|------------|-------------|
| 1 | hat 收带 6.8–9k + 起音 3ms + 降电平（原高通后敞到 Nyquist，5/5 评审头号刺耳源） | 本批 | 有意不移植倾向：trance hat 本来就该亮；若 neon 也收"刺耳"反馈再评 |
| 2 | 母带低通顶棚 7.6–11.2k→6.7–9.4k、hi-shelf −6..−1.5→−8..−3.5@2.8k | 本批 | 有意不移植：体裁频谱倾斜不同（§3） |
| 3 | sidechain duck 瞬时跳变→12ms 连续下坡（每记 kick 一次宽带咔哒） | 本批 | **待评估，优先**：neon 若同样用 `setValueAtTime` 阶跃 duck 则同 bug |
| 4 | 全乐器包络"尾闭"（stop 前 setTargetAtTime(0)；bass 残余曾 ~22%、和弦 ~25%） | 本批 | **待评估，优先**：截断咔哒是引擎通病模式，与体裁无关 |
| 5 | FM 电钢键盘亮度跟随（kbTame，高音自动纯化）+ 全表 idx 调暗 + focus 一次性音收敛 | 本批 | 待评估：neon supersaw 无 FM 结构，但 lead 高音区亮度值得对照检查 |
| 6 | 噪层补 4.2k 低通再进 limiter（原绕过全部暖化链）+ 噪带 2.4k→1.5k 移出耳敏峰 | 本批 | 待评估：查 neon 是否也有绕过母带链的直连支路 |
| 7 | 混响 IR 烘焙时一阶低通调暗（白噪 IR=亮金属尾；ConvolverNode normalize=true，常数增益惰性） | 本批 | 待评估：neon 若同样用白噪 IR 则同理适用 |
| 8 | 噪声 buffer loop 接缝 256 样本交叉淡化（消 2.4s 周期咔哒） | 本批 | **待评估，优先**：同为通病模式 |
| 9 | 军鼓 1.9k→1.5k/起音 6ms、tanh 1.3→1.15+4x 过采样+增益重排、tremolo 减半、答句只落根三五、翻面噪爆 0.5→0.3 | 本批 | 逐项体裁相关，暂不移植 |

round-2 裁定要点（防过度矫正）：暗宇宙 hat 曾被四层衰减叠没 → 低通暗端 6.4k 抬回
6.7k 对齐 hat 高通 + hat 电平微回 0.42→0.45；air 频段占比 −50dB 略暗于 Nujabes 级
参考（−38..−42），属"背景聆听少疲劳"的有意取向，若日后觉得闷，恢复杆=低通顶棚系数
（`pitchN*1800→*2000`）；若噪底觉得"太干净不像黑胶"，恢复杆=crackle 增益 0.45→0.55。
...
## 5d. 自然乐器二批（2026-07-17，双侧同落；owner：除人声外全部引入）

同 §5c 流程（2 视角实现前设计评审 → 实现 → 2 视角对抗验证双 APPROVE 零 blocker）。物理配方经评审确认
教科书级正确（拇指琴=悬臂梁本征模 1:6.27:17.55，颤音琴=拱形调音条 1:4:10）。零新增 `_rng`（棘轮 18/5 不动）；
新增 salt：lofi 54(贝斯)/55(响指)/56(竖琴)/57(康加+pattern)，neon 58(竖琴)/59(弦乐机)。

| 侧 | 新配器 | 要点 |
|----|--------|------|
| lofi | 拇指琴（10% 旋律） | 悬臂梁三模 + 指甲 tick；孤独长吟 1.3s |
| lofi | 颤音琴（10% 旋律 / 15% comp） | 马达颤 4–6Hz 每宇宙、烘焙进 buffer（绝不 playbackRate 变调）；pianoBus 防双 tremolo |
| lofi | 竖琴滚奏（35% 宇宙） | 和弦边界 +45ms 让和弦先说话；**+24 套用 mel 撞车避让 → 枚举声部严格子集**（verify 逐和弦证明） |
| lofi | 指弹电贝斯（30% 宇宙） | **混合合成**：KS 起音 + 三角波延音——纯 KS 替换实测 RMS −3.7dB（测量抓真回归），混合后 −1.1dB 性格级差异 |
| lofi | 响指 backbeat（25% 宇宙） | 1.7k Q1.5 宽带（评审：Q2.8=合成 ping 且堆回刚腾出的敏感带）；三角体保留 |
| lofi | 康加鬼音（30% 宇宙） | 奇数步 pattern 骑 swing 口袋；少数组合与 kick/fill 重合（verify 枚举确认无害） |
| neon | breakdown 竖琴琶音（40% 宇宙） | 奇数小节与偶数小节钢琴成同和弦 call-response；钢琴宇宙 +12 / 非钢琴 +24（避免同八度堆根音）+ lead 同款 min9 避让；arp 落拍让位防双拨弦 flam |
| neon | string-machine pad（40% 宇宙） | Solina 式：双锯对称 ±4¢ + 每声部反相慢漩涡 LFO 0.6+k·0.12Hz（评审：齐相快颤只是"带颤音的 pad"）+ HP160/LP2200；每声部仍恰好一次 _rngV → 隔离流形状不变 |

不变量照旧：Rhodes 保 lead 明确多数（50%）+ 永远的和弦床；neon supersaw/four-on-floor 不动；
polka/hardgroove 不配抒情乐器（wave-2 dropout 教训已内建到 harpNow 条件，verify 走查全组合无空小节）。
