...
Fur length is render-only and leaves the gait, skeleton and checked face/ear geometry untouched. Medium and long hair now use a non-uniform continuous coat profile, a cheek ruff, a bell-shaped tail plume, a soft open-flank undercoat halo, and open round-ended Bézier guard hairs interpolated between stable body/tail guides. Full detail distributes 256 medium or 380 long guard-hair curves across the silhouette; when their projected spacing becomes sub-pixel, a hysteretic LOD uses 140 or 224 wider alpha-compensated proxies instead. The six guard-hair paths and two undercoat-flank paths stay batched, never randomize per frame, and never fill closed triangles. Hairless uses a slimmer tail, muted skin palette and low-contrast folds. Fill, coat clipping, outline, shadow, hit testing and pose envelopes share the expanded silhouettes, so markings and interactions do not stop at the short-hair boundary.
